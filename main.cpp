...
int main()
{
    ...
    std::string buildCommand = "cmd.exe /c cd /d \"" + projectRoot.string() + "\" && cmake --build build --config Debug --target plug";
    ...
    while (state.window->isOpen())
    {
        ...
        // Call plugin update to render (only if function pointer is valid)
        if (plug_update && state.window)
        {
            plug_update(&state);
        }
    }
...
void reload_plug(HMODULE* libplug, plug_init_t *plug_init, plug_update_t *plug_update)
{
    ...
    *libplug = load_plug(plug_init, plug_update);
    if (*libplug && *plug_init && *plug_update)
    {
        printf("Plugin reloaded successfully! New code is active.\n");
    }
    else
    {
        printf("Failed to reload plugin! Using old code.\n");
        // Try to reload the old one as fallback
        *libplug = load_plug(plug_init, plug_update);
    }
}